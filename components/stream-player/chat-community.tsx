...
interface ChatCommunityProp {
    viewerName: string
    hostName: string
    isHidden: boolean
}

const ChatCommunity = ({ viewerName, hostName, isHidden }: ChatCommunityProp) => {
            ...
            <ScrollArea className="gap-y-2 mt-4">
                ...
                </p>
                {
                    filteredParticipants.map((participant) => (
                        <CommunityItem key={participant.identity} hostName={hostName} viewerName={viewerName} participantIdentity={participant.identity} participantName={participant.name} />
                    ))
                }
            </ScrollArea>
        </div>
